...
def scrape_page(url : str) -> object:
    ...
    while numInternalLinksToscrape > 0:
        for link in links['internal']:
            ...
            if link in linkVisited:
                continue

            soup = fetch_page(url+link)
...
''' TODO
strim down data, 
check dublicate words, 
relevant url only-not scrape all url,
if user already scraped this artist -> no need to scrape again
